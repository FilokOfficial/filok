...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FLKMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FLKMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FLKMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("filok-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("FLKMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running FLKMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadFLKMiner() exception");
    } catch (...) {
        LogPrintf("ThreadFLKMiner() exception");
    }

    LogPrintf("ThreadFLKMiner exiting\n");
}

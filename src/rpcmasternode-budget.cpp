...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a FLK initiative\n"
            "  vote-alias         - Vote on a FLK initiative\n"
            "  vote               - Vote on a FLK initiative/budget\n"
    ...
    if (strCommand == "prepare") {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start filok_address monthly_payment_filok'");
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start filok_address monthly_payment_filok fee_tx'");
